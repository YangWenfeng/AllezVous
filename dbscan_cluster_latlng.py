...
for num in [1500, 1000, 500, 250, 100, 50, 25]:
    print 'Run DBSCAN cluster with epsilon = %.4f km.' % (num / 1000.0)
    start_time = time.time()
    kms_per_radian = 6371.0088
    epsilon = num / 1000.0 / kms_per_radian
    ...
    properties_latlng_cluster['cluster_label_%dm' % num] = cluster_labels
    properties_latlng_cluster['cluster_latitude_%dm' % num] = [latitude[label] for label in cluster_labels]
    properties_latlng_cluster['cluster_longitude_%dm' % num] = [longitude[label] for label in cluster_labels]
